...
const PackageDetails = ({ setActiveStepId }: { setActiveStepId?: any }) => {
  ...
  const [editingPackage, setEditingPackage] = useState<
    BookingState["product_data"][0] | null
  >(null);
  ...
  const handleFieldChange = (
    field: keyof typeof currentForm,
    value: string
  ) => {
    if (editingPackage) {
  ...
  const handleAddToList = () => {
    ...
    const duplicate =
      ...
      packages.find((pkg) => arePackagesEqual(pkg, currentForm));
    if (duplicate) {
  ...
  const handleContinue = () => {
    ...
    if (packages.length > 0) {
      ...
      const cleanedPackages = packages.map(
        ({ id, print_type, ...rest }) => rest
      );
      dispatch(
        updateBookingField({ field: "product_data", value: cleanedPackages })
      );
    }
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputField
                ...
                name="product_type"
                onChange={(name, value) =>
                  handleFieldChange(name as keyof typeof currentForm, value)
                }
                placeholder="E.g Electronics"
              />
              <InputField
                label="Product Details"
                value={
                  editingPackage?.product_details || currentForm.product_details
                }
                name="product_details"
                onChange={(name, value) =>
                  handleFieldChange(name as keyof typeof currentForm, value)
              ...
              <InputField
                label="Product Weight (kg)"
                value={
                  editingPackage?.product_weight || currentForm.product_weight
                }
                name="product_weight"
                type="number"
                onChange={(name, value) =>
              ...
              <InputField
                label="Product Unit Value (GBP)"
                value={
                  editingPackage?.product_value || currentForm.product_value
                }
                name="product_value"
                type="number"
                onChange={(name, value) =>
                        ...
                        <tr key={pkg.id || idx} className="hover:bg-gray-50">
                          ...
                          <td className="p-2 border">
                            <div className="flex items-center gap-3">
                              ...
                              <button
                                onClick={() =>
                                  pkg.id && handleDeletePackage(pkg.id)
                                }
...
const PreviewFinish = ({ setActiveStepId }: { setActiveStepId?: any }) => {
  ...
  const toggleAdditionalService = (service: {
    ...
    const isSelected = booking.additional_services?.some(
              ...
              {additionalServices.map((service) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={booking.additional_services?.some(
                        (s) => s.name === service.name
                      )}
          ...
          <div className="bg-gray-50 p-6 rounded-lg text-sm space-y-4">
            ...
            {booking.additional_services?.map((service) => (
              <div key={service.name} className="flex justify-between">
                <span>
                  {additionalServices.find((s) => s.name === service.name)
                    ?.label || service.name}
                </span>
                <span>£{service.amount.toFixed(2)}</span>
              </div>
