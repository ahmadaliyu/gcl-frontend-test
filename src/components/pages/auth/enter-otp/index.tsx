...
import { useResendCode, useVerifyOtp, VerifyOtpResponse } from '@/services';
import { clearTempCredentials, resetForm } from '@/store/auth/formSlice';
...
function EnterOtp() {
  ...
  const password = useAppSelector((state: RootState) => state.form?.temp_credentials?.password);

  const router = useRouter();


  const { isPending, mutate } = useVerifyOtp((response: VerifyOtpResponse) => {
  ...
  return (
    <div>
      <div className="max-w-[930px] mx-auto mt-[56px]">
        <h1 className="text-[#02044A] text-center text-[36px] font-medium">Enter Otp code</h1>
